...
######################################################################
#
# Char::GBK - Source code filter to GBK script
#
...
sub Char::GBK::escape_script {
    ...
    if (/^ use Char::Egbk(?:\s+[0-9\.]*)?\s*; $/oxms) {
        ...
        $e_script .= sprintf("use Char::Egbk %s;\n%s", $Char::Egbk::VERSION, $use_re_eval); # require run-time routines version
...
Char::GBK - Source code filter to escape GBK

=head1 SYNOPSIS

  use Char::GBK;
  use Char::GBK version;         --- require version
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  m?regexp?               m?ditto@Char::Egbk::matched?
  s/regexp/replacement/   ($_ =~ m/ditto@Char::Egbk::matched/) ?
                          eval{ Char::Egbk::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  m?regexp?b              m?regexp@Char::Egbk::matched?
  s/regexp/replacement/b  ($_ =~ m/(\G[\x00-\xFF]*?)(?:regexp)@Char::Egbk::matched/) ?
                          eval{ Char::Egbk::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
