...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                for (@list) {
                    $Char::Egbk::function_ord     = 'Char::GBK::ord'     if /\A ord \z/oxms;
                    $Char::Egbk::function_ord_    = 'Char::GBK::ord_'    if /\A ord \z/oxms;
                    $Char::Egbk::function_reverse = 'Char::GBK::reverse' if /\A reverse \z/oxms;
                    $Char::Egbk::function_getc    = 'Char::GBK::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= Char::GBK::escape();
...
=head1 Escaping Multiple-Octet Code (Char::GBK software provides)
...
=head1 Multiple-Octet Anchoring of Regular Expression (Char::GBK software provides)

Char::GBK software applies multiple-octet anchoring at beginning of regular expression.
...
=head1 Escaping Second Octet (Char::GBK software provides)

Char::GBK software escapes second octet of multiple-octet character in regular
expression.
...
=head1 Multiple-Octet Character Regular Expression (Char::GBK software provides)

Char::GBK software clusters multiple-octet character with quantifier, makes cluster from
...
=head1 Calling 'Char::Egbk::ignorecase()' (Char::GBK software provides)

Char::GBK software applies calling 'Char::Egbk::ignorecase()' instead of /i modifier.
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  s/regexp/replacement/   CORE::eval{ Char::Egbk::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Char::Egbk::matched/) ?
  s/regexp/replacement/   1 :
                          CORE::eval{ Char::Egbk::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  s/regexp/replacement/b  CORE::eval{ Char::Egbk::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
...
=head1 Un-Escaping \ of \N, \p, \P, and \X (Char::GBK software provides)

Char::GBK software removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char::GBK software provide)
...
=head1 Escaping File Test Operators (Char::GBK software provide)
...
=item * Eval GBK Script
  ...
  $eval = Char::GBK::eval { block };
  $eval = Char::GBK::eval $expr;
  $eval = Char::GBK::eval;

  The Char::GBK::eval keyword serves two distinct but related purposes in JPerl.
  These purposes are represented by two forms of syntax, Char::GBK::eval { block }
  and Char::GBK::eval $expr. The first form traps runtime exceptions (errors)
  ...
  more general. Whichever form you use, Char::GBK::eval is the preferred way to do
  all exception handling in JPerl.

  For either form of Char::GBK::eval, the value returned from an Char::GBK::eval is
  ...
  Char::GBK::eval itself. See wantarray for more on how the evaluation context
  can be determined.

  If there is a trappable error (including any produced by the die operator),
  Char::GBK::eval returns undef and puts the error message (or object) in $@. If
  ...
  can test it reliably afterward for errors. A simple Boolean test suffices:
      ...
      Char::GBK::eval { ... }; # trap runtime errors
      if ($@) { ... }     # handle error

  (Prior to Perl 5.16, a bug caused undef to be returned in list context for
  syntax errors, but not for runtime errors.)

  The Char::GBK::eval { block } form is syntax checked and compiled at compile time,
  so it is just as efficient at runtime as any other block. (People familiar
  with the slow Char::GBK::eval $expr form are occasionally confused on this issue.)
  Because the { block } is compiled when the surrounding code is, this form of
  Char::GBK::eval cannot trap syntax errors.

  The Char::GBK::eval $expr form can trap syntax errors because it parses the code
  ...
  after the Char::GBK::eval is complete, as do any subroutine or format definitions.
  The code of the Char::GBK::eval is treated as a block, so any locally scoped
  variables declared within the Char::GBK::eval last only until the Char::GBK::eval is
  done. (See my and local.) As with any code in a block, a final semicolon is
  not required.

  Char::GBK::eval will be escaped as follows:

  -------------------------------------------------
  Before                  After
  -------------------------------------------------
  Char::GBK::eval { block }    eval { block }
  Char::GBK::eval $expr        eval Char::GBK::escape $expr
  Char::GBK::eval              eval Char::GBK::escape
  -------------------------------------------------

  To tell the truth, the subroutine Char::GBK::eval does not exist. If it exists,
  you will troubled, when Char::GBK::eval has a parameter that is single quoted
  string included my variables. Char::GBK::escape is a subroutine that makes Perl
  ...
  Since Char::GBK::eval traps errors that would otherwise prove fatal, it is useful
  for determining whether particular features (such as fork or symlink) are
  implemented.

  Because Char::GBK::eval { block } is syntax checked at compile time, any syntax
  error is reported earlier. Therefore, if your code is invariant and both
  Char::GBK::eval $expr and Char::GBK::eval { block } will suit your purposes equally
  well, the { block } form is preferred. For example:

      # make divide-by-zero nonfatal
      Char::GBK::eval { $answer = $a / $b; };
      warn $@ if $@;
      ...
      Char::GBK::eval { $answer = }; # WRONG

      # a runtime syntax error
      Char::GBK::eval '$answer =';   # sets $@
  ...
  Using the Char::GBK::eval { block } form as an exception trap in libraries does
  ...
  as this example shows:
      ...
      Char::GBK::eval { local $SIG{'__DIE__'}; $answer = $a / $b; };
      warn $@ if $@;
  ...
  which has the effect of changing their error messages:
      ...
      # __DIE__ hooks may modify error messages
      {
          ...
          Char::GBK::eval { die "foo lives here" };
  ...
  With an Char::GBK::eval, you should be especially careful to remember what's being
  looked at when:

      Char::GBK::eval $x;        # CASE 1
      Char::GBK::eval "$x";      # CASE 2

      Char::GBK::eval '$x';      # CASE 3
      Char::GBK::eval { $x };    # CASE 4

      Char::GBK::eval "\$$x++";  # CASE 5
      $$x++;                # CASE 6
  ...
  The block of Char::GBK::eval { block } does not count as a loop, so the loop
...
=head1 Un-Escaping bytes::* Subroutines (Char::GBK software provides)

Char::GBK software removes 'bytes::' at head of subroutine name.
...
some bugs. Thus, if you happen to find a bug that's in Char::GBK software and
