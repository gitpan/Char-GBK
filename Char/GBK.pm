...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Char::Egbk::uc($char[$i]) ne Char::Egbk::lc($char[$i]))) {
            $char[$i] = '[' . Char::Egbk::uc($char[$i]) . Char::Egbk::lc($char[$i]) . ']';
        }
...
sub e_require {
    ...
    return qq<Char::Egbk::require '$expr';>;
}
...
http://search.cpan.org/dist/Char-Char::GBK/Char::GBK/JA.pm
...
Definitions in Char/Egbk.pm.
  ...
  @{Char::Egbk::eV}             qr{(?:[\x81-\xFE][\x00-\xFF]|[^\x81-\xFE\x0C\x0A\x0D])}
...
=item * Length By GBK Character
  ...
  $string is omitted, it returns the Char::GBK::length of $_.
...
=item * Substr By GBK Character
  ...
  area past the end of the string, Char::GBK::substr raises an exception.
...
=item * Rindex By GBK Character
  ...
  $rindex = Char::GBK::rindex($string,$substring,$position);
  $rindex = Char::GBK::rindex($string,$substring);

  This function works just like Char::GBK::index except that it returns the position of
