...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (/\A (?:$q_char)*? [*?] /oxms and (my @glob = Char::Egbk::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
#
# GBK fold case lower case without parameter
#
sub Char::Egbk::fc_() {
    my $s = $_;
    return join '', map {defined($fc{$_}) ? $fc{$_} : $_} ($s =~ /\G ($q_char) /oxmsg);
...
=item File test function Char::Egbk::X
  ...
  Function and Prototype     Meaning
  ------------------------------------------------------------------------------
  Char::Egbk::s(*), Char::Egbk::s_()   File has nonzero size (returns size in bytes).
  ------------------------------------------------------------------------------
