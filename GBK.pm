...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Egbk::uc($char[$i]) ne Egbk::lc($char[$i]))) {
            $char[$i] = '[' . Egbk::uc($char[$i]) . Egbk::lc($char[$i]) . ']';
        }
...
http://search.cpan.org/dist/Char-GBK/GBK/JA.pm
...
Definitions in Egbk.pm.
  ...
  @{Egbk::eV}             qr{(?:[\x81-\xFE][\x00-\xFF]|[^\x81-\xFE\x0C\x0A\x0D])}
